...
public class NearListFragment extends LoadMoreListFragment<Pictorial> {
	/**
	 * fragment创建
	 * 
	 * @param type
	...
	public static BaseFragment<Pictorial> newInstance(String type,
		...
		final NearListFragment tf = new NearListFragment();
...
//		showpop();
	...
	public View getListItemview(View view, final Pictorial allPictorial, int position) {
		...
		TextView tv1 = (TextView) view.findViewById(R.id.listitem_youhui1);
		TextView tv2 = (TextView) view.findViewById(R.id.listitem_youhui2);
		...
		for (int i = 0; i < count; i++) {
			...
			switch (i) {
			case 0:
				tv1.setText(Double.parseDouble(p.other)*100 + "");
				...
				iv1.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						...
						intent.setClass(mContext, ShopsArticleActivity.class);
				...
				break;
			case 1:
				tv2.setText(Double.parseDouble(p.other)*100 + "");
	    		if (p.icon != null && p.icon.length() > 10) {
	    			String[] iconlist = p.icon.split(",");
	...
	public Data parseJson(String json) throws Exception {
		...
		for (int i = 0; i < count; i++) {
			...
			try {
				...
				if (obj.has("discount")) {
					o.other = obj.getString("discount");
				}
				if (obj.has("addr")) {
					o.address = obj.getString("addr");
				}
				if (obj.has("lat")) {
				...
				if (obj.has("provinceId")) {
					o.other1 = obj.getString("provinceId");
				}
				o.list_type = "1";				
			} catch (Exception e) {
			}
